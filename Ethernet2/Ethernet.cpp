...
void EthernetClass::beginMulticast(IPAddress *ifaddr, IPAddress *groupaddr)
{
	ip_addr_t ipaddr,graddr;
	uint32_t addr32,gr32;
	
	addr32 = uint32_t((*ifaddr));
	gr32 = uint32_t((*groupaddr));
	
	ip4_addr_set_u32(&ipaddr, addr32);
	ip4_addr_set_u32(&graddr, gr32);
	
	igmp_joingroup(&ipaddr, &graddr);
